...
bool telegramSentForThisAlarm = false;
...
void setup() {
  ...
  secured_client.setInsecure(); // skips certificate validation, standard for this library
  ...
  bot.sendMessage(TELEGRAM_CHAT_ID, "Fall Detection Device is now online and monitoring.", "");

  Serial.println("Fall Detection + Blynk + Telegram Started!");
}
  ...
  switch (currentState) {
    ...
    case IMPACT_DETECTED:
      if (magnitude >= STILL_LOW && magnitude <= STILL_HIGH) {
        ...
        if (now - stillnessStart >= STILLNESS_DURATION_MS) {
          ...
          telegramSentForThisAlarm = false;
    ...
    case FALL_CONFIRMED:
      ...
      if (buttonPressed) {
        ...
        Serial.println(">> No cancel - ALARM ACTIVE, sending Telegram alert");
      }
      break;

    case ALARM_ACTIVE:
      digitalWrite(BUZZER_PIN, HIGH);
      if (!telegramSentForThisAlarm) {
        bot.sendMessage(TELEGRAM_CHAT_ID, "ALERT: A fall has been detected and was NOT cancelled. Please check on the wearer immediately.", "");
        telegramSentForThisAlarm = true;
