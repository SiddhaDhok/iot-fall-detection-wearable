...
const unsigned long CANCEL_WINDOW_MS = 15000; // 15 seconds to cancel
...
void setup() {
  ...
  pinMode(BUTTON_PIN, INPUT); // KY-004 has its own pull-down, no internal pull-up needed
...
void loop() {
  ...
  bool buttonPressed = (digitalRead(BUTTON_PIN) == HIGH); // KY-004: HIGH = pressed
  ...
  switch (currentState) {
    ...
    case FALL_CONFIRMED:
      ...
      if (buttonPressed) {
        ...
        Serial.println(">> No cancel - ALARM ACTIVE (escalate here later)");
      }
      break;

    case ALARM_ACTIVE:
      digitalWrite(BUZZER_PIN, HIGH);
      // This is where Telegram/Blynk alert will fire in later steps
      if (buttonPressed) {
        currentState = CANCELLED;
        digitalWrite(BUZZER_PIN, LOW);
